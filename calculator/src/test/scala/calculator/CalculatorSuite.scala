...
class CalculatorSuite {
  ...
  @Test def `colorForRemainingCharsCount result signal should follow the input signal"`(): Unit = {
    val input = Var(52)
    val result = colorForRemainingCharsCount(input)
  ...
  @Test def `eval literal`(): Unit = {
    val input = Literal(5)
    assertEquals(5, eval(input, Map()), 0.1)
  }

  @Test def `eval ref`(): Unit = {
    val input = Plus(Ref("a"), Literal(3))
    val expressions = Map(
      "a" -> Signal[Expr](Literal(5))
    )
    assertEquals(8, eval(input, expressions), 0.1)
  }

  @Test def `eval ref not in map`(): Unit = {
    val input = Plus(Ref("b"), Literal(1))
    val expressions = Map(
      "a" -> Signal[Expr](Literal(1))
    )
    assertEquals(Double.NaN, eval(input, expressions), 0.1)
  }

  @Test def `eval self ref`(): Unit = {
    val input = Plus(Ref("b"), Literal(1))
    val expressions = Map(
      "a" -> Signal[Expr](Plus(Ref("b"), Literal(1))),
      "b" -> Signal[Expr](Times(Ref("a"), Literal(2)))
    )
    assertEquals(Double.NaN, eval(input, expressions), 0.1)
  }
