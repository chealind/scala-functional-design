...
class CodecsSuite
  ...
  @Test def `a 'List[A]' value should be encoded as a JSON array (0pts)`(): Unit = {
    val xs = 1 :: 2 :: Nil
    val encoder = implicitly[Encoder[List[Int]]](Encoder.listEncoder[Int])
    assert(encoder.encode(xs) == Json.Arr(List(Json.Num(1), Json.Num(2))))
  }

  @Test def `encode optional empty value`(): Unit = {
    val encoder = implicitly[Encoder[Option[Int]]]
    assert(encoder.encode(Option.empty) == Json.Null)
  }

  @Test def `encode optional non-empty value`(): Unit = {
    val encoder = implicitly[Encoder[Option[String]]]
    assert(encoder.encode(Option("Java")) == Json.Str("Java"))
  }

  @Test def `encode optional person value`(): Unit = {
    val encoder = implicitly[Encoder[Option[Person]]]
    assert(encoder.encode(Option(Person("Clark", 13))) == Json.Obj(Map("name" -> Json.Str("Clark"), "age" -> Json.Num(13))))
  }
  ...
  @Test def `a 'Contacts' value should be encoded as a JSON object (1pt)`(): Unit = {
    ...
    val encoder = implicitly[Encoder[Contacts]](contactsEncoder)
