...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(n) => if (n.isValidInt) Option(n.toInt) else Option.empty
      case _ => Option.empty
    }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(elements) => unfold(elements.map(decoder.decode))
      case _ => Option.empty
    }

  def unfold[A](xs: List[Option[A]]): Option[List[A]] = xs match {
    case List() => Option(List())
    case y :: ys => for (v <- y; vs <- unfold(ys)) yield v :: vs
  }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction({
      case Json.Obj(fields) => decoder.decode(fields.getOrElse(name, Json.Null))
      case _ => Option.empty
    })
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person](json => Person(json._1, json._2))
...
trait ContactsCodecs {
  ...
  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.field[List[Person]]("people")
      .transform[Contacts](json => Contacts(json))
}
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val maybePeople = parseJson(""" {"people":[{"name":"Bob","age":"66"},{"name":"Clark","age":13},{"name":"Steve","age":81}]} """)
    ...
    println(maybePeople.flatMap(_.decodeAs[Contacts]))
  }
