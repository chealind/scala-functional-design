...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] = {
    if (initial.isEmpty) LazyList()
    else {
      val neighbors: LazyList[(Block, List[Move])] = for {
        b <- initial
        n <- newNeighborsOnly(neighborsWithHistory(b._1, b._2), explored)
      } yield n
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(LazyList((Block(startPos, startPos), List())), Set())

  /**
   * Returns a lazy list of all possible pairs of the goal block along
   * with the history how it was reached.
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(e => e._1.b1 == goal && e._1.b2 == goal)
